...
cdef extern from "Netica.h":
    ...
    environ_ns* NewNeticaEnviron_ns (const str* license, environ_ns* env, const str* locn)
...
cdef class Netica:
    ...
    def NewFileStream_ns (self,char* filename, Netica environ,access=None):
        cdef char* _access
        if access:
            _access=PyUnicode_AsUTF8(access)
        else:
            _access=NULL
        res = Stream()
...
##        return [i for i in res[:size]]
    ...
    def NewLearner_bn (self, learn_method_bn method,str _options, Netica environ):
        cdef char* options
        res = Learner()
        options = PyUnicode_AsUTF8(_options) if type(_options)== str else NULL
    ...
    def SetNodeStateNames (self,NewNode node, *arg):
        ...
        for state in range(numstates):
            SetNodeStateName_bn (node.value, state, PyUnicode_AsUTF8(arg[state]))
...
## Not supported in windows
    ...
    def NewScripter_ns (self,Netica environ, char* file_name, char* language, str _options, str _script):
        ...
        cdef char* script
        options = PyUnicode_AsUTF8(_options) if type(_options)== str else NULL
        script = PyUnicode_AsUTF8(_script) if type(_script)== str else NULL
        res = Scripter ()
